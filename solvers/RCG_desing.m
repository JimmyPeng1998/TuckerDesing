function stats = RCG_desing ( A_Omega, X, A_Gamma, opts )
% RCG_desing Riemannian conjugate gradient on desingularization of the set of
% bounded Tucker rank tensors 
%
% stats = RCG_desing ( A_Omega, X, A_Gamma, opts )
% Input:
...
%   stats.X_RCG_desing: the last iterate
...
for t=1:opts.maxiter
    ...
    if t==1
        ...
        xi_old_trasp = vectortrasp_desing( Xold, X, xi_old);
        eta_old_trasp = vectortrasp_desing( Xold, X, eta_old);
        % Hestenes--Stiefel+
        diff = lincomb_desing(xi, 1, xi_old_trasp, -1);
        beta = innerprod_desing(X, diff, xi) / innerprod_desing(X, diff, eta_old_trasp);
%         beta = innerprod_desing(X, diff, xi) / innerprod_desing(X, eta_old_trasp, eta_old_trasp);
        beta = max(0, beta);
        % Search direction
        eta = lincomb_desing(xi, -1, eta_old_trasp, beta);
    end
    ...
    s = - calcInitial_mex( eucGrad.subs', eucGrad.vals, ...
        ...
        eta.tildecore.data, eta.tildeU{1}', eta.tildeU{2}', eta.tildeU{3}');
    % Retraction
    ...
    if abs(errorOmega(t+1)-errorOmega(t))/errorOmega(t)  < opts.difftol
        fprintf('RCG_desing stagnates after %d steps.\n',t)
        break
    end
    
    if errorOmega(t+1)  < opts.tol
        fprintf('RCG_desing converges after %d steps.\n',t)
        conv=true;
        break
    end
